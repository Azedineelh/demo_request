...
# try:
#     res = requests.get('https://kajvchgahejhka', timeout=(1, 2))
# except requests.exceptions.RequestException as e:
#     print(e)

# res = requests.get('https://httpbin.org/delay/5', timeout=0.5)
# print(res)

# Recuperation du token
login_request = requests.post(BASE_URL + 'auth/token', data={
    "username": "client@eventflow.test",
    "password": "client1234"
})
...
token_type = login_request.json().get('token_type')

token = login_request.json().get('access_token')

headers = {
    'Authorization': f"{token_type[0].upper()}{token_type[1:]} {token}"
}
...
order_request = requests.post(BASE_URL + 'orders', headers=headers, json=payload)

created_order = order_request.json()
